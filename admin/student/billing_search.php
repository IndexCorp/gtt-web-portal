    ...
    <div class="card">
        ...
        <div class="table-responsive border-bottom" data-toggle="lists" data-lists-values="[&quot;js-lists-values-employee-name&quot;]">
            ...
            <?php 
                ...
                if(isset($_GET['billing_search'])){
                    $term = $_GET['billing_search'];
                    $sn = 0;
                    
                    //$news = $getFromCourse->get_student_g($term);


                //  $paginations = @$getFromGeneric->get_single_g('payment', 'user_id', $news->id);
                    $paginations = @$getFromCourse->get_student_g($term);


                   foreach($paginations as $news):
                 
                    $payments = @$getFromGeneric->get_single_g('payment', 'user_id', $news->id);
                    if(empty($payments)){
                        continue;
                    }else{
