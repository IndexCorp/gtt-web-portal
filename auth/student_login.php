...
if(session_start()) session_regenerate_id(true);//session id changed
...
function sessionId(){
  ...
  session_destroy();
session_unset();
  $id = strtoupper(bin2hex(openssl_random_pseudo_bytes(26))) ;
  return $id;
...
$sessionId = session_id();
    ...
    if($user){
              ...
              $get_roles = $getFromGeneric->get_single_g('admin_role', 'admin_id', $user->id);
             // var_dump($get_role);   
            foreach($get_roles as $get_role){
              if(!empty($get_role)){
                echo '<script>alert("'.$get_role->role_id.'")</script>';
                ...
                if($role_id == 1){
                    ...
                    $_SESSION['student'] = $role_id;

                  }elseif( $role_id == 4){
                    $_SESSION['course'] = $role_id;

                  }elseif( $role_id == 5){
                   $_SESSION['community'] = $role_id;
                  }
              ...
              if($user->type == 'teacher'){
                 ...
                 header("Location: http://localhost/gtt/admin/dashboard/");
                exit();
              }else{
                  $_SESSION['login_id'] = $user->id;
                  echo 'Succcess';
                  header("Location: http://localhost/gtt/student/student-dashboard/");
                  exit();
              }
...
/*
$S = '0 - 22 1985--324';
$finals = str_replace('--','',$S);
$final = str_replace('-','',$finals);
$finals = str_replace(' ','',$final);
$finally = chunk_split($finals,3,'-');

//echo substr($finally, 0, -1);
*/ 
?>
