    ...
    class Exam extends Generic {
            ...
            public function XtractQuestion(string $filePath, string $separator=".")
            {
        ...
        public function check_marking($exam_id , $question_id, $round, $student_id){
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM marking_up WHERE `exam_id`= :exam_id AND `round` = :rounds AND `student_id` = :student_id AND `question_id` = :question_id");
        ...
        public function get_first_question_live($exam_id, $round, $student_id){
            $stmt = $this->pdo->prepare("SELECT * FROM live_question WHERE `exam_id`= :exam_id AND `round` = :rounds AND `student_id` = :student_id ORDER BY ID ASC LIMIT 1 ");
            $stmt->bindParam(":exam_id", $exam_id, PDO::PARAM_INT);
            $stmt->bindParam(":rounds", $round, PDO::PARAM_INT);
            $stmt->bindParam(":student_id", $student_id, PDO::PARAM_INT);
            $stmt->execute();
        ...
        public function get_next_question_live($exam_id, $round, $student_id, $current_id){
            $stmt = $this->pdo->prepare("SELECT * FROM live_question WHERE `exam_id`= :exam_id AND `round` = :rounds AND `student_id` = :student_id AND `id` > :current_id ORDER BY ID ASC LIMIT 1 ");
            $stmt->bindParam(":exam_id", $exam_id, PDO::PARAM_INT);
            $stmt->bindParam(":rounds", $round, PDO::PARAM_INT);
            $stmt->bindParam(":student_id", $student_id, PDO::PARAM_INT);
            $stmt->bindParam(":current_id", $current_id, PDO::PARAM_INT);
        ...
        public function get_previous_question_live($exam_id, $round, $student_id, $current_id){
            $stmt = $this->pdo->prepare("SELECT * FROM live_question WHERE `exam_id`= :exam_id AND `round` = :rounds AND `student_id` = :student_id AND `id` < :current_id ORDER BY ID DESC LIMIT 1 ");
            $stmt->bindParam(":exam_id", $exam_id, PDO::PARAM_INT);
            $stmt->bindParam(":rounds", $round, PDO::PARAM_INT);
            $stmt->bindParam(":student_id", $student_id, PDO::PARAM_INT);
            $stmt->bindParam(":current_id", $current_id, PDO::PARAM_INT);
            ...
            public function get_rand_quest($exam_id, $question_no){
                $stmt = $this->pdo->prepare("SELECT * FROM question WHERE `exam_id`= :id ORDER BY RAND() LIMIT $question_no");
                $stmt->bindParam(":id", $exam_id, PDO::PARAM_INT);
                $stmt->execute();
